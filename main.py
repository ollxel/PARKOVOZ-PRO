...
parking_spots_lock = threading.Lock()  # Блокировка для потокобезопасного доступа
marking_mode = False
current_spot_id = 0
...
async def websocket_handler(websocket):
    # УБРАТЬ ВСЕ ПРОВЕРКИ ЗАГОЛОВКОВ - они вызывают ошибки
    try:
        ...
        logging.info(f"Клиент отключен. Осталось клиентов: {len(connected_clients)}")
async def send_to_clients(data):
    ...
    for client in clients_to_send:
        try:
            ...
            logging.warning("Клиент отключился во время отправки")
            with client_lock:
                if client in connected_clients:
                    connected_clients.remove(client)
        except Exception as e:
            logging.error(f"Ошибка отправки данных клиенту: {e}")
            with client_lock:
                if client in connected_clients:
                    connected_clients.remove(client)
...
def start_websocket_server():
    ...
    try:
        ...
        logging.error(f"Ошибка WebSocket сервера: {e}")
    finally:
        ws_loop.close()
        logging.info("WebSocket сервер остановлен")
    ...
    while not stop_event.is_set():
        try:
            ...
            if frame is not None:
                # Исправление: переворачиваем изображение по вертикали
                frame = cv2.flip(frame, 0)  # 0 означает отражение по вертикали
                ...
                if last_frame is not None:
                    # Исправление: также переворачиваем last_frame перед отправкой
                    fixed_frame = cv2.flip(last_frame, 0)
                    frame_queue.put(fixed_frame.copy())
        except Exception as e:
            logging.warning(f"Ошибка загрузки изображения: {e}")
            if last_frame is not None:
                # Исправление: переворачиваем last_frame при ошибках
                fixed_frame = cv2.flip(last_frame, 0)
                frame_queue.put(fixed_frame.copy())
            time.sleep(1)
        time.sleep(0.05)
        ...
        while True:
            ...
            with parking_spots_lock:
                for i, occupied in enumerate(occupied_spots):
                    if i < len(parking_spots):
                        parking_spots[i] = (parking_spots[i][0], parking_spots[i][1], 1 if occupied else 0)
            
            # Подготавливаем данные для отправки
            with parking_spots_lock:
                spot_states = [s[2] for s in parking_spots] if parking_spots else []
                free_count = spot_states.count(0) if spot_states else 0
                occupied_count = spot_states.count(1) if spot_states else 0
...
def main():
    ...
    # Даем время на запуск WebSocket сервера
    time.sleep(1)
